...
function SessionHandler (db) {
    ...
    this.isLoggedInMiddleware = function(req, res, next) {
        ...
        sessions.getUsername(session_id, function(err, user) {
            "use strict";

            if (!err && user.username && user.fullname) {
    ...
    this.handleLoginRequest = function(req, res, next) {
        ...
        users.validateLogin(username, password, function(err, user) {
            ...
            sessions.startSession(user['_id'], user['fullname'] || '', function(err, session_id) {
                "use strict";
    ...
    function validateSignup(username, fullname, password, verify, email, errors) {
        "use strict";
        var USER_RE = /^[a-zA-Z0-9_-]{3,20}$/;
        var FULL_RE = /^[a-zA-Z ]{3,20}$/;
        var PASS_RE = /^.{3,20}$/;
        var EMAIL_RE = /^[\S]+@[\S]+\.[\S]+$/;
        ...
        if (!FULL_RE.test(fullname)) {
            errors['fullname_error'] = "invalid fullname. try just letters";
            return false;
        }
    ...
    this.handleSignup = function(req, res, next) {
        ...
        var errors = {'username': username, 'fullname': fullname, 'email': email};
        if (validateSignup(username, fullname, password, verify, email, errors)) {
            users.addUser(username, fullname, password, email, function(err, user) {
                "use strict";
                ...
                if (err) {
                    ...
                    } else if(err.message == 'NUM_USER_BIGGER_THAN_ZERO') {
                        errors['username_error'] = "The maximum number of user has been reach";
                        return res.render("signup", errors);
                    }
